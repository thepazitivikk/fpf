...
access_granted = "🔓 Доступ к ПУ предоставлен!\n\n" \
                 ...
                 "⚙️ Чтобы открыть меню настроек <i>FunPay Functional</i>, введи команду /menu."
...
✅ <i>FPF</i> получает полную историю чата и видит все данные о всех новых сообщениях.
✅ <i>FPF</i> может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
✅ <i>FPF</i> может точно определить, кто именно написал, будь то ты, твой собеседник или 3е лицо (арбитраж).
❌ Из-за того что <i>FPF</i> получает полную историю чата для обнаружения новых сообщений, чат становится "прочитанным" (не загорается оранжевым).
...
❌ <i>FPF</i> не получает полную историю чата, вследствие чего видит только последнее сообщение, а не все. Если пользователь быстро напишет несколько сообщений, <i>FPF</i> увидит только последнее.
❌ <i>FPF</i> не может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
❌ <i>FPF</i> не может точно определить, кто именно написал: ты или собеседник. Если чат не прочитан, значит сообщение от собеседника, иначе - от тебя. Однако, если ты смотришь чат в момент получения сообщений, данная логика иногда может сбоить. Так же <i>FPF</i> не сможет определить, что в чат написало 3е лицо (арбитраж).

❗ Если нажать кнопку <code>Больше</code> в уведомлении о новом сообщении, <i>FPF</i> "прочитает" чат и покажет последние 15 сообщений, в том числе изображения. Так же <i>FPF</i> сможет определить, кто именно автор сообщений."""
...
❌ <i>FunPay Functional</i> еще <b><u>не инициализирован</u></b> и <b><u>никакие его функции не работают</u></b>.\n
🔃 Как только <i>FunPay Functional</i> инициализируется, данное сообщение изменится.\n
📋 Если <i>FPF</i> долго не инициализируется, проверь логи с помощью команды /logs"""

fpc_init = """✅ <b><u>FunPay Functional инициализирован!</u></b>\n
...
🔄 <b><i>Обновления:</i></b> @FPF_updates
🧩 <b><i>Плагины:</i></b> @FPF_plugins
👨‍💻 <b><i>Разработчик:</i></b> @woopertail, @sidor0912, @thepazitivik
🤑 <b><i>Био:</i></b> @biopazik"""
...
about = """<b>🐦 FunPay Functional 🐦 v{}</b>\n
<i>Telegram чат:</i> @funpay_cardinal
<i>Обновления:</i> @FPF_updates
<i>Плагины:</i> @FPF_plugins
<i>Разработчик:</i> @woopertail, @sidor0912, @thepazitivik
<i>Био:</i> @biopazik"""
...
<b>ЦП:</b>
{}
    Используется <i>FPF</i>: <code>{}%</code>
...
update_done = "✅ Установил обновление! Перезапусти <i>FPF</i> с помощью команды /restart"
update_done_exe = "✅ Установил обновление! Новый <code>FPF.exe</code> находится в папке <code>update</code>. " \
                  "Выключи <i>FPF</i>, перенеси новый <code>FPF.exe</code> на место старого " \
...
pl_new = "Отправь мне плагин.\n\n<b>⚠️ ВНИМАНИЕ! Загрузка плагинов из сомнительных источников может привести к печальным последствиям.\n" \
         "@FPF_plugins решает большинство возможных проблем.</b>"

au_user_settings = "Настройки для пользователя {}"
adv_FPF = "😎 FunPay Functional - лучший бот для FunPay"
...
🛠️ Сделано с помощью: github.com/thepazitivikk/fpf
👨‍💻 Автор: @woopertail, @sidor0912, @thepazitivik
💰 Био: @biopazik
🔄 Обновления: @FPF_updates
🧩 Плагины: @FPF_plugins
💬 Чат: @funpay_cardinal"""
...
desc_lang = desc_main
desc_gs = "Здесь ты можешь включать и выключать основные функции <i>FPF</i>."
...
cmd_restart = "перезапустить FPF"
cmd_power_off = "выключить FPF"
